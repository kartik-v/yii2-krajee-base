...
/**
 * Html5Input widget is a widget encapsulating the HTML 5 inputs for Bootstrap 4.
 *
 * @author Kartik Visweswaran <kartikv2@gmail.com>
 * @since 1.0
 * @see http://twitter.github.com/typeahead.js/examples
 */
class Html5Bs4Input extends Html5Input
{
    /**
    ...
    /**
     * @var string one of the SIZE modifiers 'lg', 'md', 'sm'
     */
    public $size;
    ...
    {
        $view = $this->getView();
        Html5Bs4InputAsset::register($view);
     ...
     * @param string $type the addon type (prepend or append)
     *
     * @return string
     */
    protected static function getAddonContent($addon, $type = 'prepend')
    {
        if (is_array($addon)) {
            $content = ArrayHelper::getValue($addon, 'content', '');
            $options = ArrayHelper::getValue($addon, 'options', []);
            Html::addCssClass($options, "input-group-{$type}");
            if (!ArrayHelper::getValue($addon, 'asButton', false)) {
